import { useState, useEffect, KeyboardEvent, RefObject, useRef } from "react";
import {
  Appointment,
  AppointmentFormData,
  SlotDateTime,
...
export const useAppointments = () => {
  ...
  const handleCreateAppointment = (
    ...
    if (appointmentData.tipoCita === "solicitud") {
      ...
      title = `[${appointmentData.tipoCita.toUpperCase()}] ${
        appointmentData.orden?.cliente || "Cliente"
...
export const useCreateAppointmentForm = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.tipoCita === "solicitud") {
      ...
      title = `[${formData.tipoCita.toUpperCase()}] ${
        ordenParaGuardar?.cliente || "Cliente"
