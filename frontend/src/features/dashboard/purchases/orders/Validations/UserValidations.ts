...
import { purchaseOrder, formErrors, formTouched, purchaseOrderBase } from "../types/typesPurchaseOrder";
...
export const validateAllFields = (
  ...
  // Si hay items, precioUnitario y cantidad se validan a través de los items (o se ignoran aquí)
  ...
  const errors: formErrors = {
    ...
    precioUnitario: hasItems ? '' : validateField('precioUnitario', formData.precioUnitario, formDataWithDefaults, isEditMode),
    fecha: validateField('fecha', formData.fecha, formDataWithDefaults, isEditMode),
    cantidad: hasItems ? '' : validateField('cantidad', formData.cantidad || 1, formDataWithDefaults, isEditMode),
...
export const validateSpecificFields = (
  ...
  fields.forEach(field => {
    if (field in formDataWithDefaults) {
      const value = (formDataWithDefaults as any)[field];
