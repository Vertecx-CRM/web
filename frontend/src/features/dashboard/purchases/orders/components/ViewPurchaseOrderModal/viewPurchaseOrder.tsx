...
export const ViewPurchaseOrderModal: React.FC<viewPurchaseOrderModalProps> = ({
    ...
    let subtotal = 0;
    if (hasItems) {
        subtotal = purchaseOrder.items!.reduce((acc, item) => acc + (item.cantidad * item.precioUnitario), 0);
    } else {
                            ...
                            {hasItems ? (
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200">
                                        <thead className="bg-gray-50">
                                            <tr>
                                                ...
                                                <th scope="col" className="px-3 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Cant.</th>
                                                <th scope="col" className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Precio Unit.</th>
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {purchaseOrder.items!.map((item, index) => (
                                                <tr key={index}>
                                                    <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900">{item.producto}</td>
                                                    <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-500 text-center">{item.cantidad}</td>
                                                    <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-500 text-right">${item.precioUnitario.toLocaleString('es-CO')}</td>
                                                    <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900 text-right">${(item.cantidad * item.precioUnitario).toLocaleString('es-CO')}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="w-full px-3 py-2 bg-white border border-gray-300 rounded-md">
                                            ${(purchaseOrder.precioUnitario || 0).toLocaleString('es-CO')}
                                        </div>
                                    </div>
                                </div>
                            )}
