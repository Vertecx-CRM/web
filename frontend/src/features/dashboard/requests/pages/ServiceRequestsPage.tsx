...
export default function ServiceRequestsPage() {
  ...
  const columns: Column<Row>[] = [
    { key: "id", header: "Id" },
    { key: "descripcion", header: "Descripcion", render: (r) => <div className="max-w-[280px] truncate md:max-w-none md:whitespace-normal">{r.descripcion}</div> },
  ...
  function setDate(row: Row) {
    ...
    setRows((prev) => prev.map((r) => (r.id === row.id ? { ...r, fecha: val } : r)));
  }
    ...
    <RequireAuth>
      <main className="flex-1 flex flex-col bg-gray-100">
        <div className="px-4 pt-6">
          <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-[#CC0000]">Solicitudes</h1>
        </div>
